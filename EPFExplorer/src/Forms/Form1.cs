...
namespace EPFExplorer
{
    public partial class Form1 : Form
    {
        ...
        public string[] langs = new string[] { "English", "Spanish", "Italian", "French", "German", "Dutch", "Japanese", "Chinese", "Korean", "Hangul", "Norwegian", "Swedish", "UKEnglish" };
        string[] languagePermutablePaths = new string[] { "/levels/"};
        ...
        public void createPermutables(string s, List<string> output) {
            foreach (string permutablePath in languagePermutablePaths)
            {
                if (s.Contains(permutablePath))
                {
                    foreach (string lang in langs)
                    {
                        output.Add(s.Replace(permutablePath, permutablePath + lang + "/"));
                    }
                }
            }
        }
        ...
        public Form1()
        {
            ...
            string[] langSuffixes = new string[langs.Length];

            for (int i = 0; i < langs.Length; i++) {
                langSuffixes[i] = "_"+langs[i].Substring(0, 2).ToUpper();
            }

            List<string> levelsLanguagePermutations = new List<string>();

            foreach (string s in stringsEPF){
                createPermutables(s, levelsLanguagePermutations);
                //foreach (string langsuffix in langSuffixes) {   //SLOW - only use when searching for multilanguage files in a rom you haven't done it to yet. Then reactivate all commented out references to form1.masterListOfSuccessfulMultilangSuffixNames and add its contents, after loading the arc file, to form1.stringsEPF so that you don't have to use this slow method in future.
                //    levelsLanguagePermutations.Add(s + langsuffix);
                //}
            }

            foreach (string s in stringsHR){
                createPermutables(s, levelsLanguagePermutations);
                levelsLanguagePermutations.Add(s + langSuffixes);
               //foreach (string langsuffix in langSuffixes) {    //SLOW - only use when searching for multilanguage files in a rom you haven't done it to yet. Then reactivate all commented out references to form1.masterListOfSuccessfulMultilangSuffixNames and add its contents, after loading the arc file, to form1.stringsEPF so that you don't have to use this slow method in future.
               //    levelsLanguagePermutations.Add(s + langsuffix);
               //}
            }
        ...
        //public string masterListOfSuccessfulMultilangSuffixNames = "";
